...
public class ManagePark2 extends StringPrompt {
	...
	// Constructor
	public ManagePark2(InspiredNations instance, Player playertemp, int errortemp, String parkname) {
		plugin = instance;
		player = playertemp;
	...
	public ManagePark2(InspiredNations instance, Player playertemp, int errortemp, Vector<String> namestemp, String parkname) {
		plugin = instance;
		player = playertemp;
		PDI = plugin.playerdata.get(player.getName().toLowerCase());
		town = PDI.getTownMayored();
	...
	public String getPromptText(ConversationContext arg0) {
		...
		if (error == 5) {
			errormsg = errormsg.concat(ChatColor.RED + "That park name is already being used in this town.");
		}
		if (error == 6) {
	...
	public Prompt acceptInput(ConversationContext arg0, String arg) {
		...
		if (!input.contains(args[0].toLowerCase())) return new ManagePark2(plugin, player, 1, name);
		...
		if (args.length < 2) return new ManagePark2(plugin, player, 2, name);
		
		// Add Builder
		if (args[0].equalsIgnoreCase("add")){
			if (find(args[2]).size() == 1) args[2] = find(args[2]).get(0);
			else if (find(args[2]).size() == 0) return new ManagePark2(plugin, player, 6, name);
			else return new ManagePark2(plugin, player, 9, find(args[2]), name);
			
			park.addBuilder(args[2]);
			return new ManagePark2(plugin, player, 0, name);
		}
		
		// Remove Builder
		if (args[0].equalsIgnoreCase("remove")) {
			if (find(args[2], park.getBuilders()).size() == 1) args[2] = find(args[2], park.getBuilders()).get(0);
			else if (find(args[2], park.getBuilders()).size()== 0) return new ManagePark2(plugin, player, 3, name);
			else return new ManagePark2(plugin, player, 9, find(args[2], park.getBuilders()), name);
		...
		if (args[0].equalsIgnoreCase("rename")) {
			...
			for (Park parktest: town.getParks()) {
				if (parktest.getName().equalsIgnoreCase(tempname) && !parktest.equals(park)) {
					return new ManagePark2(plugin, player, 5, name);
				}
			}
			park.setName(tempname);
			return new ManagePark2(plugin, player, 0, tempname);
		}
		return new ManagePark2(plugin ,player, 0, name);
	}
