...
public class ClaimFederalPark1 extends StringPrompt {
	...
	// Constructor
	public ClaimFederalPark1(InspiredNations instance, Player playertemp, int errortemp) {
		plugin = instance;
		player = playertemp;
	...
	public String getPromptText(ConversationContext arg0) {
		...
		String main = ChatColor.BOLD + "Claim Park: Pick your selection type." + ChatColor.RESET + repeat(" ", 5)+ ChatColor.DARK_AQUA + repeat("-", 53) + ChatColor.YELLOW;
		String options = "";
	...
	public Prompt acceptInput(ConversationContext arg0, String arg) {
		...
		if (!input.contains(arg.toLowerCase())) return new ClaimFederalPark1(plugin, player, 1);
		PM.clear();
		// Cuboid
		if (arg.equalsIgnoreCase("cuboid")) {
			PM.setCuboid(new Cuboid(player.getLocation().getWorld().getName()));
