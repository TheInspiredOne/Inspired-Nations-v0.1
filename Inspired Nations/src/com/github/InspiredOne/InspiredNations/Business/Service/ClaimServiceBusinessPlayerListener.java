package com.github.InspiredOne.InspiredNations.Business.Service;
...
public class ClaimServiceBusinessPlayerListener {
	...
	public ClaimServiceBusinessPlayerListener(InspiredNations instance, PlayerMoveEvent event) {
		plugin = instance;
	...
	public ClaimServiceBusinessPlayerListener(InspiredNations instance, PlayerInteractEvent event) {
		plugin = instance;
		player = event.getPlayer();
		playername = player.getName().toLowerCase();
		PDI = plugin.playerdata.get(playername);
	...
	public String generateMessage() {
		...
		if (PM.reSelectServiceBusiness) {
			main = ChatColor.BOLD + "Reclaim Business: Type what you would like to do." + ChatColor.RESET + repeat(" ", 5)+ ChatColor.DARK_AQUA + repeat("-", 53) + ChatColor.YELLOW;
		}
		else {
			main = ChatColor.BOLD + "New Business: Type what you would like to do." + ChatColor.RESET + repeat(" ", 5)+ ChatColor.DARK_AQUA + repeat("-", 53) + ChatColor.YELLOW;
		}
		String options = "";
