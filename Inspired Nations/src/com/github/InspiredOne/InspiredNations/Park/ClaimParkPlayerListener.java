...
public class ClaimParkPlayerListener {
	...
	public ClaimParkPlayerListener(InspiredNations instance, PlayerMoveEvent event) {
		plugin = instance;
	...
	public ClaimParkPlayerListener(InspiredNations instance, PlayerInteractEvent event) {
		plugin = instance;
		player = event.getPlayer();
		playername = player.getName().toLowerCase();
		PDI = plugin.playerdata.get(playername);
	...
	public void onPlayerMove() {
		if (!PM.parkSelect() || !(PM.isSelectingCuboid() || PM.isSelectingPolygon())) return;
	...
	public String generateMessage() {
		...
		BigDecimal expenditures = cut(new BigDecimal(size * PDI.getTownMayored().getNationTax()/10000 * plugin.getConfig().getDouble("park_tax_multiplyer")));
		...
		String end = ChatColor.GOLD + "" + size + ChatColor.YELLOW + " Cubic Meters. " + repeat(" ", 55 - sizeLength)  + "Cost: " + ChatColor.GOLD + expenditures + 
				ChatColor.YELLOW + " " + PDI.getPluralMoney() + " per year." + repeat(" ", 20 - expendituresLength) + ChatColor.DARK_AQUA + repeat("-", 53)
				+ ChatColor.AQUA + " Type 'exit' to leave or 'back' to go back.";
		if (PM.isSelectingCuboid()) {
