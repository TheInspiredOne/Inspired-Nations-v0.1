...
public class ReclaimServiceBusiness2 extends StringPrompt {
	...
	// Constructor
	public ReclaimServiceBusiness2(InspiredNations instance, Player playertemp, int errortemp, ServiceBusiness busin) {
		plugin = instance;
		player = playertemp;
	...
	public String getPromptText(ConversationContext arg0) {
		...
		BigDecimal cost = cut(new BigDecimal(size * PDI.getServiceBusinessTax() / 100.0));
		
		String space = ChatColor.DARK_AQUA + repeat(" ", plugin.getConfig().getInt("hud_pre_message_space")) + ChatColor.GOLD;
		String main = ChatColor.BOLD + "Reclaim Business: Type what you would like to do." + ChatColor.RESET + repeat(" ", 5)+ ChatColor.DARK_AQUA + repeat("-", 53) + ChatColor.YELLOW;
		String options = "";
	...
	public Prompt acceptInput(ConversationContext arg0, String arg) {
		...
		if (!input.contains(arg.toLowerCase())) return new ReclaimServiceBusiness2(plugin, player, 1, business);
		...
		if (arg.equalsIgnoreCase("back")) {
			...
			return new ReclaimServiceBusiness1(plugin, player, 0, business);
		}
		...
		if (arg.equalsIgnoreCase("finish")) {
			...
			if (PM.isSelectingPolygon()) {
				...
				if (!isSimple(PM.getPolygon().getPolygon())) {
					...
					return new InvalidLand(plugin, player, 1, "reserviceBusiness", business);
				}
				if (PM.getPolygon().Volume() == 0) {
					PM.setBlocksBack();
					PM.serviceBusiness(false);
					return new InvalidLand(plugin, player, 4, "reserviceBusiness", business);
				}
					...
					for (int j = (int) rect.getMinY() - 1; j <= (int)rect.getMaxY() + 1; j++) {
						for (int l = PM.getPolygon().getYMin() - 1; l <= PM.getPolygon().getYMax() + 1; l++) {
							...
							if ((!town.isIn(test)) && PM.getPolygon().isIn(test)) {
								PM.setBlocksBack();
								PM.serviceBusiness(false);
								return new InvalidLand(plugin, player, 3, "reserviceBusiness", business);
							}
							if (PM.getPolygon().Volume() == 0) {
								PM.setBlocksBack();
								PM.serviceBusiness(false);
								return new InvalidLand(plugin, player, 2, "reserviceBusiness", business);
							}
							...
							for (Iterator<ServiceBusiness> k = town.getServiceBusinesses().iterator(); k.hasNext();) {
								...
								if (businesstemp.isIn(test) && PM.getPolygon().isIn(test)){
									if (!businesstemp.getName().equals(business.getName())) {
										PM.setBlocksBack();
										PM.serviceBusiness(false);
										return new InvalidLand(plugin, player, 2, "reserviceBusiness", business);
									}
				...
				business.setPolySpace(PM.getPolygon());
			...
			if (PM.isSelectingCuboid()) {
				...
				business.setCubeSpace(PM.getCuboid());
			}
			PM.selectCuboid(false);
			PM.selectPolygon(false);
			PM.serviceBusiness(false);
			...
			return new ManageBusiness2(plugin, player, 0, business.getName());
		}
