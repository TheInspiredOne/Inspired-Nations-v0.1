...
public class NewServiceBusiness2 extends StringPrompt {
	...
	// Constructor
	public NewServiceBusiness2(InspiredNations instance, Player playertemp, int errortemp) {
		plugin = instance;
		player = playertemp;
	...
	public Prompt acceptInput(ConversationContext arg0, String arg) {
		...
		if (!input.contains(arg.toLowerCase())) return new NewServiceBusiness2(plugin, player, 1);
		...
		if (arg.equalsIgnoreCase("finish")) {
			...
			if (PM.isSelectingPolygon()) {
				...
				if (!isSimple(PM.getPolygon().getPolygon())) {
					...
					return new InvalidLand(plugin, player, 1, "serviceBusiness");
				}
				if (PM.getPolygon().Volume() == 0) {
					PM.setBlocksBack();
					PM.serviceBusiness(false);
					return new InvalidLand(plugin, player, 4, "serviceBusiness");
				}
					...
					for (int j = (int) rect.getMinY() - 1; j <= (int)rect.getMaxY() + 1; j++) {
						for (int l = PM.getPolygon().getYMin() - 1; l <= PM.getPolygon().getYMax() + 1; l++) {
							...
							if ((!town.isIn(test)) && PM.getPolygon().isIn(test)) {
								PM.setBlocksBack();
								PM.serviceBusiness(false);
								return new InvalidLand(plugin, player, 3, "serviceBusiness");
							}
							if (PM.getPolygon().Volume() == 0) {
								PM.setBlocksBack();
								PM.serviceBusiness(false);
								return new InvalidLand(plugin, player, 2, "serviceBusiness");
							}
