...
public class NewHouse2 extends StringPrompt{
	...
	// Constructor
	public NewHouse2(InspiredNations instance, Player playertemp, int errortemp) {
		plugin = instance;
		player = playertemp;
	...
	public String getPromptText(ConversationContext arg0) {
		...
		String main = ChatColor.BOLD + "New House: Type what you would like to do." + ChatColor.RESET + repeat(" ", 5)+ ChatColor.DARK_AQUA + repeat("-", 53) + ChatColor.YELLOW;
		String options = "";
	...
	public Prompt acceptInput(ConversationContext arg0, String arg) {
		...
		if (!input.contains(arg.toLowerCase())) return new NewHouse2(plugin, player, 1);
		...
		if (arg.equalsIgnoreCase("finish")) {
			...
			if (PM.isSelectingPolygon()) {
				...
				if (PM.getPolygon().Volume() == 0) {
					PM.setBlocksBack();
					PM.house(false);
					return new InvalidLand(plugin, player, 4, "house");
				}
					...
					for (int j = (int) rect.getMinY() - 1; j <= (int)rect.getMaxY() + 1; j++) {
						for (int l = PM.getPolygon().getYMin() - 1; l <= PM.getPolygon().getYMax() + 1; l++) {
							...
							if ((!town.isIn(test)) && PM.getPolygon().isIn(test)) {
								PM.setBlocksBack();
								PM.house(false);
								return new InvalidLand(plugin, player, 3, "house");
							}
							if (PM.getPolygon().Volume() == 0) {
								PM.setBlocksBack();
								PM.house(false);
								return new InvalidLand(plugin, player, 2, "house");
							}
