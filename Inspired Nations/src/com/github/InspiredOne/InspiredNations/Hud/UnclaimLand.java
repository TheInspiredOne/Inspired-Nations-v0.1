...
public class UnclaimLand extends StringPrompt {
	...
	// Constructor
	public UnclaimLand(InspiredNations instance, Player playertemp, int errortemp) {
		plugin = instance;
		player = playertemp;
	...
	public String getPromptText(ConversationContext arg0) {
		...
		BigDecimal expenditurestemp = cut(new BigDecimal(country.size()*(plugin.getConfig().getDouble("base_cost_per_chunk"))));
		BigDecimal expenditures = cut(expenditurestemp.multiply(country.getMoneyMultiplyer()));
		int expendituresLength = (int) (("" + expenditures).length() * 1.4);

		int maxClaimableChunks = CM.getMaxClaimableChunks();
		...
		for (int z = -9; z < 4; z++) {
			for (int x = -26; x < 27; x++) {
				...
				if (z == 0 && x == 0) {
					...
					if (area.isIn(chunk, spot.getWorld().getName())) {
	...
	public Prompt acceptInput(ConversationContext arg0, String arg) {
		...
		if (!input.contains(arg.toLowerCase())) return new UnclaimLand(plugin, player, 1);
