...
public class ManageFederalPark1 extends StringPrompt {
	...
	public ManageFederalPark1(InspiredNations instance, Player playertemp, int errortemp, Vector<String> parktemp) {
		plugin = instance;
		player = playertemp;
		PDI = plugin.playerdata.get(player.getName().toLowerCase());
		country = PDI.getCountryRuled();
	...
	public Vector<String> find(String name) {
		...
		Vector<Park> park = country.getParks();
	...
	public Prompt acceptInput(ConversationContext arg0, String arg) {
		...
		if (find(arg).size() == 1) {
			arg = find(arg).get(0);
			return new ManageFederalPark2(plugin, player, 0, arg);
		}
		else if(find(arg).isEmpty()) {
			return new ManageFederalPark1(plugin, player, 1);
		}
		else return new ManageFederalPark1(plugin, player, 2, find(arg));
	}
