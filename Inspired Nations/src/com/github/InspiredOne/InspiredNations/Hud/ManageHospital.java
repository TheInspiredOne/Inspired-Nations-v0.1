...
public class ManageHospital extends StringPrompt {
	...
	// Constructor
	public ManageHospital(InspiredNations instance, Player playertemp, int errortemp) {
		plugin = instance;
		player = playertemp;
		...
		error = errortemp;
		hospital = town.getHospital();
	}
	
	public ManageHospital(InspiredNations instance, Player playertemp, int errortemp, Vector<String> namestemp) {
		plugin = instance;
		player = playertemp;
		PDI = plugin.playerdata.get(player.getName().toLowerCase());
		town = PDI.getTownMayored();
	...
	public Vector<String> find(String name) {
		...
		for (Iterator<String> i = players.iterator(); i.hasNext();) {
			...
			if (nametest.toLowerCase().contains(name.toLowerCase()) && !nametest.equalsIgnoreCase(player.getName()) && !hospital.getBuilders().contains(nametest)) {
				list.add(nametest);
	...
	public String getPromptText(ConversationContext arg0) {
		...
		String main = ChatColor.BOLD + "Manage Hospital: Type what you would like to do." + ChatColor.RESET + repeat(" ", 1)+ ChatColor.DARK_AQUA + repeat("-", 53) + ChatColor.YELLOW;
		String options = "";
		...
		options = options.concat(ChatColor.YELLOW + "Builders: " + ChatColor.GOLD + format(hospital.getBuilders())+" ");
		options = options.concat(ChatColor.DARK_AQUA + repeat("-",53) + ChatColor.GREEN);
		options = options.concat("Add Builder <player>" + repeat(" ", 50));
		if(hospital.getBuilders().size() != 0 ){
	...
	public Prompt acceptInput(ConversationContext arg0, String arg) {
		...
		if (args[0].equalsIgnoreCase("add")){
			...
			else if (find(args[2]).size() == 0) return new ManageHospital(plugin, player, 6);
			else return new ManageHospital(plugin, player, 9, find(args[2]));
			
			hospital.addBuilder(args[2]);
			return new ManageHospital(plugin, player, 0);
		}
		
		// remove
		if (args[0].equalsIgnoreCase("remove")) {
			if (find(args[2], hospital.getBuilders()).size() == 1) args[2] = find(args[2], hospital.getBuilders()).get(0);
			else if (find(args[2], hospital.getBuilders()).size()== 0) return new ManageHospital(plugin, player, 3);
			else return new ManageHospital(plugin, player, 9, find(args[2], hospital.getBuilders()));
