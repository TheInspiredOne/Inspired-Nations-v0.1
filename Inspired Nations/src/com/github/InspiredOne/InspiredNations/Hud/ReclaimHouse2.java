...
public class ReclaimHouse2 extends StringPrompt{
	...
	// Constructor
	public ReclaimHouse2(InspiredNations instance, Player playertemp, int errortemp, House housetemp) {
		plugin = instance;
		player = playertemp;
	...
	public String getPromptText(ConversationContext arg0) {
		...
		BigDecimal cost = cut(new BigDecimal(size * PDI.getHouseTax() / 100.0));
		
		String space = ChatColor.DARK_AQUA + repeat(" ", plugin.getConfig().getInt("hud_pre_message_space")) + ChatColor.GOLD;
		String main = ChatColor.BOLD + "Reclaim House: Type what you would like to do." + ChatColor.RESET + repeat(" ", 5)+ ChatColor.DARK_AQUA + repeat("-", 53) + ChatColor.YELLOW;
		String options = "";
		...
		if (PM.isSelectingPolygon()) {
			options = options.concat("Use a " + name.toLowerCase() + "to select the corners of your house. The lowest selection will be the bottom and the highest selection" +
	...
	public Prompt acceptInput(ConversationContext arg0, String arg) {
		...
		if (!input.contains(arg.toLowerCase())) return new ReclaimHouse2(plugin, player, 1, house);
		...
		if (arg.equalsIgnoreCase("finish")) {
			...
			if (PM.isSelectingPolygon()) {
				...
				if (!isSimple(PM.getPolygon().getPolygon())) {
					PM.setBlocksBack();
					PM.house(false);
					return new InvalidLand(plugin, player, 1, "rehouse", house);
				}
				if (PM.getPolygon().Volume() == 0) {
					PM.setBlocksBack();
					PM.house(false);
					return new InvalidLand(plugin, player, 4, "rehouse", house);
				}
					...
					for (int j = (int) rect.getMinY() - 1; j <= (int)rect.getMaxY() + 1; j++) {
						for (int l = PM.getPolygon().getYMin() - 1; l <= PM.getPolygon().getYMax() + 1; l++) {
							...
							if ((!town.isIn(test)) && PM.getPolygon().isIn(test)) {
								PM.setBlocksBack();
								PM.house(false);
								return new InvalidLand(plugin, player, 3, "rehouse", house);
							}
							if (PM.getPolygon().Volume() == 0) {
								PM.setBlocksBack();
								PM.house(false);
								return new InvalidLand(plugin, player, 2, "rehouse", house);
							}
							for (Iterator<House> k = town.getHouses().iterator(); k.hasNext();) {
								...
								if (housetemp.isIn(test) && PM.getPolygon().isIn(test)){
									if(!housetemp.getName().equalsIgnoreCase(house.getName())) {
										PM.setBlocksBack();
										PM.house(false);
										return new InvalidLand(plugin, player, 2, "rehouse", house);
									}
