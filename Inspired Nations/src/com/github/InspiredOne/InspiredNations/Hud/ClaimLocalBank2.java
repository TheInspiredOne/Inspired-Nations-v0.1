...
public class ClaimLocalBank2 extends StringPrompt {
	...
	// Constructor
	public ClaimLocalBank2(InspiredNations instance, Player playertemp, int errortemp) {
		plugin = instance;
		player = playertemp;
	...
	public Prompt acceptInput(ConversationContext arg0, String arg) {
		...
		if (!input.contains(arg.toLowerCase())) return new ClaimLocalBank2(plugin, player, 1);
		...
		if (arg.equalsIgnoreCase("finish")) {
			...
			if (PM.isSelectingPolygon()) {
				...
				if (!isSimple(PM.getPolygon().getPolygon())) {
					PM.setBlocksBack();
					PM.localBank(false);
					return new InvalidLand(plugin, player, 1, "localbank");
				}
				if (PM.getPolygon().Volume() == 0) {
					PM.setBlocksBack();
					PM.localBank(false);
					return new InvalidLand(plugin, player, 4, "localbank");
				}
					...
					for (int j = (int) rect.getMinY() - 1; j <= (int)rect.getMaxY() + 1; j++) {
						for (int l = PM.getPolygon().getYMin() - 1; l <= PM.getPolygon().getYMax() + 1; l++) {
							...
							if ((!town.isIn(test)) && PM.getPolygon().isIn(test)) {
								PM.setBlocksBack();
								PM.localBank(false);
								return new InvalidLand(plugin, player, 3, "localbank");
							}
							if (PM.getPolygon().Volume() == 0) {
								PM.setBlocksBack();
								PM.localBank(false);
								return new InvalidLand(plugin, player, 2, "localbank");
							}
				...
				town.setBank(new LocalBank(plugin, PM.getPolygon(), town.getCountry(), plugin.countrydata.get(town.getCountry().toLowerCase()).getTowns().indexOf(town)));
			}
			...
			if (PM.isSelectingCuboid()) {
				...
				town.setBank(new LocalBank(plugin, PM.getCuboid(), town.getCountry(), plugin.countrydata.get(town.getCountry().toLowerCase()).getTowns().indexOf(town)));
			}
			PM.selectCuboid(false);
			PM.selectPolygon(false);
			PM.localBank(false);
