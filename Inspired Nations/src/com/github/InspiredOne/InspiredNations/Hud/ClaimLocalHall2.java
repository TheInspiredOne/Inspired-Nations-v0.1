...
public class ClaimLocalHall2 extends StringPrompt{
	...
	// Constructor
	public ClaimLocalHall2(InspiredNations instance, Player playertemp, int errortemp) {
		plugin = instance;
		player = playertemp;
	...
	public String getPromptText(ConversationContext arg0) {
		...
		String main = ChatColor.BOLD + "Claim Town Hall: Type what you would like to do." + ChatColor.RESET + repeat(" ", 5)+ ChatColor.DARK_AQUA + repeat("-", 53) + ChatColor.YELLOW;
		String options = "";
		...
		if (PM.isSelectingPolygon()) {
			options = options.concat("Use a " + name.toLowerCase() + "to select the corners of your town hall. The lowest selection will be the bottom and the highest selection" +
	...
	public Prompt acceptInput(ConversationContext arg0, String arg) {
		...
		if (!input.contains(arg.toLowerCase())) return new ClaimLocalHall2(plugin, player, 1);
		...
		if (arg.equalsIgnoreCase("finish")) {
			...
			if (PM.isSelectingPolygon()) {
				...
				if (!isSimple(PM.getPolygon().getPolygon())) {
					PM.setBlocksBack();
					PM.localHall(false);
					return new InvalidLand(plugin, player, 1, "localhall");
				}
				if (PM.getPolygon().Volume() == 0) {
					PM.setBlocksBack();
					PM.localHall(false);
					return new InvalidLand(plugin, player, 4, "localhall");
				}
					...
					for (int j = (int) rect.getMinY() - 1; j <= (int)rect.getMaxY() + 1; j++) {
						for (int l = PM.getPolygon().getYMin() - 1; l <= PM.getPolygon().getYMax() + 1; l++) {
							...
							if ((!town.isIn(test)) && PM.getPolygon().isIn(test)) {
								PM.setBlocksBack();
								PM.localHall(false);
								return new InvalidLand(plugin, player, 3, "localhall");
							}
							if (PM.getPolygon().Volume() == 0) {
								PM.setBlocksBack();
								PM.localHall(false);
								return new InvalidLand(plugin, player, 2, "localhall");
							}
				...
				town.setTownHall(new LocalHall(plugin, PM.getPolygon(), town.getCountry(), plugin.countrydata.get(town.getCountry().toLowerCase()).getTowns().indexOf(town)));
			}
			...
			if (PM.isSelectingCuboid()) {
				...
				town.setTownHall(new LocalHall(plugin, PM.getCuboid(), town.getCountry(), plugin.countrydata.get(town.getCountry().toLowerCase()).getTowns().indexOf(town)));
			}
			PM.selectCuboid(false);
			PM.selectPolygon(false);
			PM.localHall(false);
