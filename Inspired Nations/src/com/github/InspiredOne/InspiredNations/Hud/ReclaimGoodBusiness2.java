...
public class ReclaimGoodBusiness2 extends StringPrompt {
	...
	ItemStack item;
	int error;
	GoodBusiness business;
	
	// Constructor
	public ReclaimGoodBusiness2(InspiredNations instance, Player playertemp, int errortemp, GoodBusiness busin) {
		plugin = instance;
		player = playertemp;
	...
	public Prompt acceptInput(ConversationContext arg0, String arg) {
		...
		if (!input.contains(arg.toLowerCase())) return new ReclaimGoodBusiness2(plugin, player, 1, business);
		...
		if (arg.equalsIgnoreCase("back")) {
			...
			return new ReclaimGoodBusiness1(plugin, player, 0, business);
		}
		...
		if (arg.equalsIgnoreCase("finish")) {
			...
			if (PM.isSelectingPolygon()) {
				...
				if (!isSimple(PM.getPolygon().getPolygon())) {
					...
					return new InvalidLand(plugin, player, 1, "regoodbusiness", business);
				}
				if (PM.getPolygon().Volume() == 0) {
					PM.setBlocksBack();
					PM.goodBusiness(false);
					return new InvalidLand(plugin, player, 4, "regoodbusiness", business);
				}
					...
					for (int j = (int) rect.getMinY() - 1; j <= (int)rect.getMaxY() + 1; j++) {
						for (int l = PM.getPolygon().getYMin() - 1; l <= PM.getPolygon().getYMax() + 1; l++) {
							...
							if ((!town.isIn(test)) && PM.getPolygon().isIn(test)) {
								PM.setBlocksBack();
								PM.goodBusiness(false);
								return new InvalidLand(plugin, player, 3, "regoodbusiness", business);
							}
							if (PM.getPolygon().Volume() == 0) {
								PM.setBlocksBack();
								PM.goodBusiness(false);
								return new InvalidLand(plugin, player, 2, "regoodbusiness", business);
							}
