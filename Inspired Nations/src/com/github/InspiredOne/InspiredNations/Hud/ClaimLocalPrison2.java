...
public class ClaimLocalPrison2 extends StringPrompt {
	...
	// Constructor
	public ClaimLocalPrison2(InspiredNations instance, Player playertemp, int errortemp) {
		plugin = instance;
		player = playertemp;
	...
	public String getPromptText(ConversationContext arg0) {
		...
		String main = ChatColor.BOLD + "Claim Prison: Type what you would like to do." + ChatColor.RESET + repeat(" ", 5)+ ChatColor.DARK_AQUA + repeat("-", 53) + ChatColor.YELLOW;
		String options = "";
		String end = ChatColor.GOLD + "" + size + ChatColor.YELLOW + " Cubic Meters. " + ChatColor.DARK_AQUA + repeat("-", 53) + ChatColor.AQUA + " Type 'exit' to leave or 'back' to go back." + repeat(" ", 25);
		...
		if (PM.isSelectingPolygon()) {
			options = options.concat("Use a " + name.toLowerCase() + "to select the corners of your prison. The lowest selection will be the bottom and the highest selection" +
	...
	public Prompt acceptInput(ConversationContext arg0, String arg) {
		...
		if (!input.contains(arg.toLowerCase())) return new ClaimLocalPrison2(plugin, player, 1);
		...
		if (arg.equalsIgnoreCase("finish")) {
			...
			if (PM.isSelectingPolygon()) {
				...
				if (!isSimple(PM.getPolygon().getPolygon())) {
					...
					return new InvalidLand(plugin, player, 1, "localprison");
				}
				if (PM.getPolygon().Volume() == 0) {
					PM.setBlocksBack();
					PM.localPrison(false);
					return new InvalidLand(plugin, player, 4, "localprison");
				}
					...
					for (int j = (int) rect.getMinY() - 1; j <= (int)rect.getMaxY() + 1; j++) {
						for (int l = PM.getPolygon().getYMin() - 1; l <= PM.getPolygon().getYMax() + 1; l++) {
							...
							if ((!town.isIn(test)) && PM.getPolygon().isIn(test)) {
								PM.setBlocksBack();
								PM.localPrison(false);
								return new InvalidLand(plugin, player, 3, "localprison");
							}
							if (PM.getPolygon().Volume() == 0) {
								PM.setBlocksBack();
								PM.localPrison(false);
								return new InvalidLand(plugin, player, 2, "localprison");
							}
				...
				town.setPrison(new LocalPrison(plugin, PM.getPolygon(), town.getCountry(), plugin.countrydata.get(town.getCountry().toLowerCase()).getTowns().indexOf(town)));
			}
			...
			if (PM.isSelectingCuboid()) {
				...
				town.setPrison(new LocalPrison(plugin, PM.getCuboid(), town.getCountry(), plugin.countrydata.get(town.getCountry().toLowerCase()).getTowns().indexOf(town)));
			}
			PM.selectCuboid(false);
			PM.selectPolygon(false);
			PM.localBank(false);
